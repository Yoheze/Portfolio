...
export default function Contact() {
  ...
  const validateMessage = (message) => {
    ...
    return (
      message.length >= 0 ||
      (regex.test(message) && message.length <= maxLength)
    );
  };
  ...
  const handleSubmit = async (e = 0) => {
    ...
    if (!validateMessage(message)) {
      setErrorMessage(
        "Invalid message. Please remove any special characters and ensure the message is under 500 characters."
      );
      return;
    }
  ...
  const check =
    ...
    errorMessage === "OAuth Login successful!" ? (
      <p style={{ color: "lightgreen" }}>{errorMessage}</p>
    ) : (
  ...
  const messageLimit =
    message.length > 500 ? (
      <p style={{ color: "red" }}>{message.length}/500</p>
    ) : (
      ...
      >
        ...
        <div className={styles.textarea}>
          ...
          <textarea
            ...
            placeholder="Any message you want to send, limited to 500 letters"
          ></textarea>
          {messageLimit}
        </div>
        ...
        {check}
        <p>
          Click these buttons below if you want to quickly send me your name and
          email!
        </p>
